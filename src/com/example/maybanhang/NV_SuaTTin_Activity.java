...
public class NV_SuaTTin_Activity extends TabActivity {

	private Spinner sp_trangthaiNV, sp_gioitinhNV, sp_chinhanhNV, sp_chucdanhNV,
			sp_bophanNV, sp_hopdongNV;
	...
	private EditText edt_ngaysinhNV, edt_ngaycapcmtndDNV, edt_ngaylamNV,
			...
			edt_datconNV;
	private Button btn_suaNV, btn_suanv_exit;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setContentView(R.layout.nv_sua_main);
		...
		btn_suanv_exit = (Button) findViewById(R.id.btn_suanv_exit);
		...
		try {
			...
			nhanvien_get = (NV_NhanVien)intent.getSerializableExtra("nhanvien");
		} catch (Exception e) {
		...
		for (int i = 0; i < ttnv_arr.length; i++) {
			if(nhanvien_get.getTrangThai().equals(ttnv_arr[i]))
				posittion_sp = i;
		}
		sp_trangthaiNV.setSelection(posittion_sp);
		...
		for (int i = 0; i < giotinh_arr.length; i++) {
			if (nhanvien_get.getGioiTinh().equals(giotinh_arr[i]))
				posittion_sp = i;
			sp_gioitinhNV.setSelection(posittion_sp);
		}
		...
		for (int i = 0; i < chinhanh_arr.size(); i++) {
			if(nhanvien_get.getChiNhanh().equals(chinhanh_arr.get(i)))
				posittion_sp = i;
		}
		sp_chinhanhNV.setSelection(posittion_sp);
		
		// chuc danh		
		ArrayAdapter<String> chucdanh_adt = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, chucdanh_arr);
		chucdanh_adt.setDropDownViewResource(android.R.layout.simple_list_item_single_choice);
		sp_chucdanhNV.setAdapter(chucdanh_adt);
		for (int i = 0; i < chucdanh_arr.length; i++) {
			if(nhanvien_get.getChucDanh().equals(chucdanh_arr[i]))
				posittion_sp = i;
			sp_chucdanhNV.setSelection(posittion_sp);
		}

		// bo phan		
		ArrayAdapter<String> bophan_adt = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, bophan_arr);
		bophan_adt.setDropDownViewResource(android.R.layout.simple_list_item_single_choice);
		sp_bophanNV.setAdapter(bophan_adt);
		for (int i = 0; i < bophan_arr.length; i++) {
			if(nhanvien_get.getBoPhan().equals(bophan_arr[i]))
				posittion_sp = i;
			sp_bophanNV.setSelection(posittion_sp);
		}

		// loai hopp dong		
		ArrayAdapter<String> hopdong_adt = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, hopdong_arr);
		hopdong_adt.setDropDownViewResource(android.R.layout.simple_list_item_single_choice);
		sp_hopdongNV.setAdapter(hopdong_adt);
		for (int i = 0; i < hopdong_arr.length; i++) {
			if(nhanvien_get.getLoaiHopDong().equals(hopdong_arr[i]))
				posittion_sp = i;
			sp_hopdongNV.setSelection(posittion_sp);
		}
		...
		btn_suanv_exit.setOnClickListener(new MyEvent());
		
		// set date
		//setCurrentDateOnView();
		edt_ngaycapcmtndDNV.setText(nhanvien_get.getNgayCap());
		edt_ngaylamNV.setText(nhanvien_get.getNgayLam());
		edt_ngaynghiNV.setText(nhanvien_get.getNgayNghi());
		edt_ngaysinhNV.setText(nhanvien_get.getNgaySinh());
		
		edt_tkNV.setText(nhanvien_get.getMaNV());
		edt_mkNV.setText(nhanvien_get.getMatKhau());
		edt_matheNV.setText(nhanvien_get.getMaThe());
		edt_hotenNV.setText(nhanvien_get.getHoTen());
		edt_quequanNV.setText(nhanvien_get.getQueQuan());
		edt_cmtndNV.setText(nhanvien_get.getCMTND());
		edt_noicapcmtndNV.setText(nhanvien_get.getNoiCap());
		edt_ghichuNV.setText(nhanvien_get.getGhiChu());
		edt_hsluongNV.setText(nhanvien_get.getHSLuong());
		edt_phucapNV.setText(nhanvien_get.getTroCap());
		edt_thangthuongNV.setText(nhanvien_get.getThangThuong());
		edt_thangphatNV.setText(nhanvien_get.getThangPhat());
		edt_datconNV.setText(nhanvien_get.getDatCoc());
	}
	...
	private class MyEvent implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.edt_ngaynghiNV:
				...
				break;
			case R.id.btn_suaNV:
				Sua_ThongTin_NhanVien();
				break;
			case R.id.btn_suanv_exit:
				Intent intent = new Intent(NV_SuaTTin_Activity.this,
	...
	private void Sua_ThongTin_NhanVien()
	{
		NV_NhanVien nhanvien = new NV_NhanVien();
		nhanvien.setID(nhanvien_get.getID());
		...
		if (database.updateNhanVien(nhanvien) > 0) {
			Intent intent = new Intent(NV_SuaTTin_Activity.this,NV_MainActivity.class);
			startActivity(intent);
			Toast toast = Toast.makeText(this, "Đã cập nhật thông tin nhân viên thành công!",
							Toast.LENGTH_SHORT);
			toast.show();
		} else {
			Toast toast = Toast.makeText(this,"Không thể cập nhật thông tin nhân viên lúc này, thử lại sau!",
					Toast.LENGTH_SHORT);
			toast.show();
		}
	}
